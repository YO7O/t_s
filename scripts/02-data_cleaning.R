...
cleaned_data <-
  ...
  group_by(city, date) |>
  summarise(shelter_program_with_space = sum(unoccupied_rooms != 0),
            total_shelter_program = n(),
            percentage_full = mean(unoccupied_rooms == 0) * 100
            )

cleaned_data |> filter(city == "Toronto")
